...
from apps.commons.get_querysets import get_students_queryset, get_group_queryset, get_student_in_group_queryset, \
    get_course_queryset, get_teacher_queryset, get_attendance_queryset, get_rent_books_queryset, get_lessons_queryset
    ...
    def get(self, request, *args, **kwargs):
        ...
        match page:
            ...
            case 'groups':
                context.update({
                    'page': 'groups',
                    'teachers': get_teacher_queryset().values('id', 'first_name'),
            ...
            case 'lessons':
                context.update({
                    'page': 'lessons',
                    'group_id': group_id,
                    'lessons': get_lessons_queryset().filter(group_id=group_id, group__teacher_id=teacher_id).annotate(
            ...
            case 'group_attendancy':
                ...
                if not group:
                    raise Http404
                group_students = get_students_queryset().filter(
                    student_in_group__group_id=group_id,
                )
                group_attendance = get_attendance_queryset().filter(
            ...
            case 'students':
                context.update({
                    'page': 'students',
                    'students': get_students_queryset().values('id', 'first_name', 'last_name', 'phone_number', 'is_verified').order_by('-id')
                })
            ...
            case 'teachers':
                context.update({
                    'page': 'teachers',
                    'teachers': get_teacher_queryset().annotate(
            ...
            case 'books':
                context.update({
                    'page': 'books',
                    'rent_books': get_rent_books_queryset()
                })
        ...
        match method:
            ...
            case 'add.group':
                ...
                with transaction.atomic():
                    group_students = []
                    if online_students_ids:
                        for online_students_id in online_students_ids:
                            group_students.append(
                                GroupStudent(
                                    student_id=online_students_id, group_id=obj.id, student_type='online', creator_id=user.id
                                    )
                                )
                    if offline_students_ids:
                        for offline_students_id in offline_students_ids:
                            group_students.append(
                                GroupStudent(
                                    student_id=offline_students_id, group_id=obj.id, student_type='offline', creator_id=user.id
                                    )
                                )
                    if group_students:
